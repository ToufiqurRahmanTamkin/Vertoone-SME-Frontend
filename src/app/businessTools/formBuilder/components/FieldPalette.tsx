...
import type { FieldCatalogue, FormFieldDefinition } from "@/types/domain/formBuilder";
import { useDraggable } from "@dnd-kit/core";
import { GripVertical, Plus } from "lucide-react";
import { FieldIcon } from "./FieldIcon";
import { groupDefinitions } from "../formBuilder.utils";
...
interface FieldPaletteProps {
  ...
  disabled: boolean;
  onAdd: (definition: FormFieldDefinition) => void;
}
  ...
  return (
    ...
    >
      ...
      <span className="flex size-8 shrink-0 items-center justify-center rounded-md border bg-background text-muted-foreground">
        <FieldIcon name={definition.icon} icons={icons} />
      </span>
...
export function FieldPalette({ catalogue, disabled, onAdd }: FieldPaletteProps) {
  const groups = groupDefinitions(catalogue.fields);

  return (
    <div className="space-y-4">
      <p className="text-[11px] text-muted-foreground">
        Drag a question onto the outline, or use the plus to drop it at the end.
      </p>
