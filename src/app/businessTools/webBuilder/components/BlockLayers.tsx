...
import type { Block, BlockCatalogue } from "@/types/domain/webBuilder";
    ...
    >
      ...
      >
        <span className="flex size-7 shrink-0 items-center justify-center rounded-md border bg-background text-muted-foreground">
          <BlockIcon name={icon} icons={icons} className="size-3.5" />
        </span>
        <span className="min-w-0 flex-1">
          <span className="block truncate text-xs font-medium">{label}</span>
          <span className="block truncate text-[11px] text-muted-foreground">
            {block.hidden ? "Hidden" : block.layout.background.toLowerCase().replace("_", " ")}
          </span>
        </span>
      </button>
  ...
  if (blocks.length === 0) {
    return (
      <div className="rounded-lg border border-dashed p-6 text-center text-xs text-muted-foreground">
        No sections yet. Add one from the library to start the page.
      </div>
    );
  }
