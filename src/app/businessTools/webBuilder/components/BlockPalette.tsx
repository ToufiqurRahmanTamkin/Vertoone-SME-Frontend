...
import type { BlockCatalogue, BlockDefinition } from "@/types/domain/webBuilder";
import { useDraggable } from "@dnd-kit/core";
import { GripVertical, Plus } from "lucide-react";
import { BlockIcon } from "./BlockIcon";
import { groupDefinitions } from "../webBuilder.utils";
  ...
  return (
    ...
    >
      ...
      <span className="flex size-8 shrink-0 items-center justify-center rounded-md border bg-background text-muted-foreground">
        <BlockIcon name={definition.icon} icons={icons} />
      </span>
...
export function BlockPalette({ catalogue, disabled, onAdd }: BlockPaletteProps) {
  const groups = groupDefinitions(catalogue.blocks);

  return (
    <div className="space-y-4">
      <p className="text-[11px] text-muted-foreground">
        Drag a section onto the outline, or use the plus to drop it at the end.
      </p>
