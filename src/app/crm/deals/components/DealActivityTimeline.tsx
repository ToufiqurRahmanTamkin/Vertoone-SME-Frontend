...
const ICONS: Record<DealActivity["type"], LucideIcon> = {
  NOTE: StickyNote,
  CALL: Phone,
  EMAIL: Mail,
  MEETING: Users,
...
export function DealActivityTimeline({
  ...
  const { data, isLoading } = useGetDealActivitiesQuery({
    dealId,
    limit: 100,
    sortBy: "occurredAt",
    sortOrder: "desc",
  ...
  const [updateActivity] = useUpdateDealActivityMutation();
  const [deleteActivity, { isLoading: isDeleting }] = useDeleteDealActivityMutation();
  const [pendingDelete, setPendingDelete] = React.useState<DealActivity | null>(null);

  const activities = React.useMemo(() => data?.data ?? [], [data]);

  const groups = React.useMemo(() => {
    const byDay = new Map<string, DealActivity[]>();
  ...
  const complete = async (activity: DealActivity) => {
    try {
  ...
  if (activities.length === 0) {
    return (
      <p className="rounded-lg border border-dashed px-4 py-8 text-center text-sm text-muted-foreground">
        Nothing logged yet. Record the first call, demo or note.
      </p>
    );
  }
  ...
  return (
    <>
                      ...
                      <div className="mt-2 flex flex-wrap items-center gap-1.5">
                        ...
                        {activity.outcome !== "NONE" && (
                          <Badge variant="secondary" className="text-[11px]">
                            {DEAL_ACTIVITY_OUTCOME_LABELS[activity.outcome]}
                          </Badge>
                        )}
      ...
      <ConfirmDialog
        ...
        description="It disappears from the timeline. The deal and its stage history are untouched."
        confirmText="Remove"
        variant="destructive"
