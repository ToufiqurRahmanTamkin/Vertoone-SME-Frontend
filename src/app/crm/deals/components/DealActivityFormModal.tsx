...
import {
  ...
  DEAL_ACTIVITY_OUTCOME_LABELS,
  DEAL_ACTIVITY_OUTCOMES,
  DEAL_ACTIVITY_TYPE_LABELS,
  type DealActivity,
} from "@/types/domain/deal";
import { DealActivitySchema, type DealActivityFormValues } from "@/validations/deal";
...
interface DealActivityFormModalProps {
  open: boolean;
...
const TYPE_OPTIONS = DEAL_ACTIVITY_MANUAL_TYPES.map((type) => ({
  label: DEAL_ACTIVITY_TYPE_LABELS[type],
  value: type,
}));

const OUTCOME_OPTIONS = DEAL_ACTIVITY_OUTCOMES.map((outcome) => ({
  label: DEAL_ACTIVITY_OUTCOME_LABELS[outcome],
  value: outcome,
}));

const emptyValues = (): DealActivityFormValues => ({
  type: "CALL",
  subject: "",
  body: "",
  location: "",
...
export function DealActivityFormModal({
  open,
  onOpenChange,
  dealId,
  activity,
}: DealActivityFormModalProps) {
  const isEdit = Boolean(activity);

  const { data: employeeOptions = [] } = useGetEmployeeOptionsQuery();

  const [createActivity, { isLoading: isCreating }] = useCreateDealActivityMutation();
  const [updateActivity, { isLoading: isUpdating }] = useUpdateDealActivityMutation();
  const isSaving = isCreating || isUpdating;

  const form = useForm<DealActivityFormValues>({
    resolver: zodResolver(DealActivitySchema),
  ...
  React.useEffect(() => {
    ...
    form.reset(
      activity
        ? {
            type: activity.type as DealActivityFormValues["type"],
  ...
  const onSubmit = async (values: DealActivityFormValues) => {
    try {
      const body = {
        type: values.type,
      ...
      if (activity) {
        ...
        await createActivity({ ...body, dealId }).unwrap();
